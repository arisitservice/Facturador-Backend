...
public class CancellationReasonRepository : ICancellationReasonRepository
{
    ...
    public CancellationReasonRepository(IHttpContextAccessor context)
    {
        dbContext = context.HttpContext.Items[Constants.HttpContextTenantDbContextKey] as TenantDbContext;
    }

    public async Task<IEnumerable<CancellationReason>> GetAllAsync()
    {
        return await dbContext.CancellationReasons.ToListAsync();
    }
}
